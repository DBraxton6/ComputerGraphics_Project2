...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
 ...
 void savePts(String fn) 
  ...
  void displaySkater() {
      ...
      pt[] S = new pt [nv];           // for shoulder/arm placement
      ...
      if(showSkater) {
        ...
        fill(yellow); caplet(P(H[f], -15, normOut), 5, P(S[f], -15, normOut), 10); //right arm
        fill(yellow); sphere(P(H[f], -15, normOut), 5); //right hand
